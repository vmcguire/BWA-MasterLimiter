# BWA-MasterLimiter — Read this before doing anything
...
can clone, browse, and reason about BWA-MasterLimiter on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-MasterLimiter happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/masterlimiter/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-MasterLimiter.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-MasterLimiter," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-MasterLimiter.vst3`
...
- **Role:** Master true-peak limiter.
- **Pain attacked:** True-peak BS.1770. 3 algorithms. Streaming presets. Per-band option.
- **Scope verdict:** master-only
- **Band tier behavior:** Per-band limiting at higher tiers.

Full row in
[`MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)
— search for `BWA-MasterLimiter`.
...
2. **[`BWA-Architecture/docs/USER-STORY-MAP.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/USER-STORY-MAP.md)**
   — where BWA-MasterLimiter sits in the bedroom-producer journey
   (Stage 8 — MASTER).
3. **[`BWA-Architecture/docs/PLAN-2026-Q2.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/PLAN-2026-Q2.md)**
   — current build plan + repo inventory. Find BWA-MasterLimiter in §9 and
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-MasterLimiter. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/masterlimiter/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-MasterLimiter row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-MasterLimiter.vst3` that:
...
```bash
git subtree push --prefix=plugins/masterlimiter \
  git@github.com:vmcguire/BWA-MasterLimiter.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-MasterLimiter.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-MasterLimiter._
